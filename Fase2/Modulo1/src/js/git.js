...
async function repos(user) {
  let response = await fetch(`https://api.github.com/users/${user}/repos?per_page=4`);
  
  let data = await response.json();
  console.log(data);

  // const detalles = document.querySelector('#app');

  data.forEach(element => {
      const card = document.createElement('datails');
      card.className = "card";
      const img = document.createElement('img');
      img.src = element.avatar_url;
